  ...
  while (true) {
    try {
      ...
      if ((isRateLimit || isNetworkError) && currentTry <= retries) {
        ...
        console.warn(`OpenRouter ${reason}. Retrying attempt ${currentTry}/${retries} in ${delay.toFixed(0)}ms...`);
        await wait(delay);
        continue;
      }
      throw error;
...
async function chatCompletion(
  ...
  if (!response.ok) {
    ...
    const err: any = new Error(`OpenRouter API error: ${response.status} - ${errorBody}`);
